...
class ScanParameters:
	...
	def get_ra_dec(**kwargs):
		telescope_loc = ephem.Observer()
		if 'obs' in kwargs:
			#obs is an ephem.Observer()
		...
		for i in range (0, n_stp):
			...
			#first step of the scan: makes sure we're not appending to an empty array
				...
				ra_min, dec_min = crd.hor_to_eq(a_min, e_min, float(telescope_loc.lat), telescope_loc.sidereal_time())
				ra_max, dec_max = crd.hor_to_eq(a_max, e_max, float(telescope_loc.lat), telescope_loc.sidereal_time())
				#print ra_min, ra_max
